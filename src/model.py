...
class Base(nn.Module):
    ...
    def fit(self, dataloader, x_train, M_train, Z_train, x_test, M_test, Z_test, target_outcome, logger):
        losses = []
        logger.debug(
        ...
        for epoch in range(200):
            ...
            for (x, y, z) in dataloader:
                ...
                if self.args.alpha > 0.0:
                    ...
                    i_id = np.where(
                        (z.cpu().detach().numpy() == zi).all(axis=1))[0]
                    zjs = zuniq.tolist()
                    ...
                    for (i, (zj)) in enumerate(zjs):
                        j_id = np.where(
                            (z.cpu().detach().numpy() == zj).all(axis=1))[0]
                        # ipm += mmd_lin(x_rep_stack[2][i_id], x_rep_stack[2][j_id], p=len(i_id)/(len(i_id)+len(j_id)))
                        ipm += mmd_rbf(x_rep[i_id], x_rep[j_id], p=len(i_id) / (len(i_id) + len(j_id)),
                                       sig=self.args.sig)
            ...
            if epoch % 10 == 0:
                ...
                logger.debug('[Epoch: %d] [%.3f], [%.3f, %.3f, %.3f], [%.3f, %.3f, %.3f] ' %
                             (epoch, epoch_loss,
